...
const DepositHistory: React.FC<DepositHistoryProps> = ({ onBack }) => {
  ...
  const fetchTransactions = async (pageNum = 1) => {
    try {
      const response = await fetch(`/api/wallet/transactions?type=deposit&page=${pageNum}&limit=10`, {
        headers: {
      ...
      <div className="p-4">
        {transactions.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-gray-600 text-lg font-medium mb-6">No deposit history available.</p>
            <button
              onClick={goHome}
