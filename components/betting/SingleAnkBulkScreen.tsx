...
interface SingleAnkBulkScreenProps {
  onBack: () => void;
  gameName: string;
}
...
const SingleAnkBulkScreen: React.FC<SingleAnkBulkScreenProps> = ({ onBack, gameName }) => {
  const { user } = useAuth();
  ...
  const addBetEntry = () => {
    ...
    if (!/^[0-9]$/.test(currentNumber)) {
      alert('Please enter a valid single digit (0-9)');
      return;
    }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white shadow-sm border-b border-gray-200">
        <div className="flex items-center justify-between p-4">
          ...
          <div className="flex-1 text-center">
            <h1 className="text-lg font-bold text-gray-800">NIGHT- SINGLE ANK</h1>
          </div>
          <WalletChip size="sm" />
        </div>
      </div>
      ...
      <div className="bg-white border-b border-gray-200 p-4">
        <div className="text-center">
          <h2 className="text-xl font-bold text-gray-800 mb-3">Single Ank - Bulk</h2>
          ...
          <div className="bg-white rounded-lg border border-gray-200 overflow-hidden mb-4">
            ...
            {betEntries.map((entry) => (
              <div key={entry.id} className="p-4 border-b border-gray-200 grid grid-cols-4 gap-3 items-center text-center">
                ...
                <span className="text-sm text-gray-600 font-medium">{entry.type}</span>
                <button
...
export default SingleAnkBulkScreen;
